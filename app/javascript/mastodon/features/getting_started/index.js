...
class GettingStarted extends ImmutablePureComponent {
  ...
  render () {
    ...
    if (multiColumn) {
      ...
      navItems.push(<ColumnLink icon='nico' text='ニコニコ形式' href='/nicomment' external={true} />);

      if (!columns.find(item => item.get('id') === 'TAG')) {
        navItems.push(<ColumnLink icon='nico' text='動画タイムライン' to='/timelines/tag/ニコニコ動画タイムライン' />);
      }

      if (!columns.find(item => item.get('id') === 'TAG')) {
        navItems.push(<ColumnLink icon='nico' text='生放送タイムライン' to='/timelines/tag/ニコニコ生放送タイムライン' />);
      }
